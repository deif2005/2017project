...
modelApp.controller('modelListCtrl', function ($scope) {
    ...
    function updateQuesStat(){
        ...
        for(var i=1;i<$obj.length;i++){
            ...
            if($ques.find('input:eq(1)').val()!==''&&$ques.find('.tree-sec').length!==0) {
                quesscoreInt = parseFloat(quesscoreInt)+(parseFloat($ques.find('input:eq(1)').val())*parseFloat($ques.find('.tree-sec').length));
            }else{
                quesscoreInt = parseFloat(quesscoreInt)+0;
            }
        }
    ...
    $('.step-first').on('click','.next-tree .add-ques',function(){
        ...
        MY.ajax({
            ...
            type:'post',
            data:{page:1,subjectId:$scope.paper.subjectId,questionTypeId:typeId,checkStatus:2,knowId:knowledges[0].id}
        }).done(function(data) {
    ...
    function verifyOk(){
        ...
        if(parseFloat(quesscore).toFixed(2)!=parseFloat($scope.paper.totalScore).toFixed(2)){
            MY.alert("题目分数合计与试卷总分不一致！",'warning');
            return;
        }
        var has = (parseFloat(quesscore).toFixed(2))-(parseFloat($scope.paper.passScore).toFixed(2));
        if(has<0){
        ...
        var obj = $('#knowledgeId').val();
        var knowledgeString = obj.join(',');
        json['id'] = $('.paperName').attr('data-pid');
            ...
            if (data.rtnCode == "000000") {
                $scope.$apply(function() {
                    ...
                    for (var i = 0; i < knowledgeList.length; i++) {
                        var id = knowledgeList[i].id
                        ids.push(id);
                    }
                    $('.knowledgeSel').val(ids).trigger("change");
                    $('.common-input[name="paperTimes"]').val(data.bizData.paper.length);
                    var datalist=[];
                ...
                for(var key in qMap){
                    for(var obj in sList){
                        ...
                        datalist.push(json);
                            ...
                            scorelength = parseFloat(scorelength)+(parseFloat(qMap[key].length)*parseFloat(sList[obj].score));
                        }
                    }
                }
                    var jsons = {typeNum:sList.length,score:scorelength.toFixed(2),quesNum:count};
