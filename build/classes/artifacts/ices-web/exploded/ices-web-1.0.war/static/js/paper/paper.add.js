...
modelApp.controller('modelListCtrl', function ($scope,$rootScope) {
    ...
    $('.step-first').on('click','.next-tree .add-ques',function(){
        ...
        MY.ajax({
            ...
            type:'post',
            data:{page:1,subjectId:$scope.subjectId,questionTypeId:typeId}
        }).done(function(data) {
                ...
                MY.ajax({
                    ...
                    if (data.rtnCode == "000000") {
                        ...
                        for(var i=0;i<list.length;i++){
                            $div.find('.third-list').append('<div class="third-se"><div class="question-cell"><input style="margin:0;" name="radio"  type="radio" value="1"/></div><div class="question-cell">'+list[i].content+'</div></div>');
                        }
