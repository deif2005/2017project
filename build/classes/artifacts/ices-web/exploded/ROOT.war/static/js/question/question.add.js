...
modelApp.controller('modelListCtrl', function ($scope) {
		var _this = {
			init: function() {
				...
				$("#knowledgeId").select2();
				//初始化题型和知识点
				this.subjectChange(null,function() {
					var questionTypeId = $('#questionTypeIdHidden').val();
					$scope.selectedTypeId = questionTypeId;
				});
				//设置表单验证规则
			...
			typeChange: function(e,type) {
				var type = type?type:$(this).find('option:selected').data('code');
				console.log(type);
				//清空选项面板
			...
			addOptions: function(e,content) {
				...
				switch (type) {
					...
					//填空题区域
					case 104:
						_this.createInsertPanel(content,true);
						break;
					//主观题区域
					case 105:
						_this.createInsertPanel(content,false);
						break;
					default :
						break;
				}
			...
			createInsertPanel: function(content,isObjectiveQT) {
				//统计页面已添加选项数量
				var count = _this.$answerPanel.find('.answer-panel-child').length;
				if (isObjectiveQT ==true){
			...
			subjectChange: function(e,_callback) {
				...
				$.when(ajax1,ajax2).done(function(data1,data2) {
					$scope.$apply(function() {
						...
						var type = data2[0].bizData[0].code;
						if( $scope.selectedTypeId) {
							$.each(data2[0].bizData, function(i,obj) {
								if(obj.id == $scope.selectedTypeId) {
									type = obj.code;
								}
							});
						}
						_this.typeChange(null,type);
					});
				});
			},
			...
			submitFunc: function() {
				...
				//收集选项值json
				var options = _this.getQuestionOptionsStr(qtypeCode);
				if(qtypeCode != '104'&&qtypeCode != '105' && options.length == 0) {
				...
				//收集标答json
				var answers = _this.getQuestionAnswerStr(qtypeCode);
				dataJson.questionContent.questionAnswerStr = JSON.stringify(answers);
				if(qtypeCode != '104'&&qtypeCode != '105') {
					if(answers.length == 0) {
					...
					if(qtypeCode == '104'&& tks.length != answers.length) {
				...
				MY.ajax({
					type:'POST',
					url:'/question/addQuestionDetail',
